...
from ..models import (
    Recipe, RecipeEquipment, RecipeFermentable, RecipeHop, RecipeMisc, RecipeYeast
)
...
def update_recipe(
    ...
    if payload.style is not None: recipe.style = payload.style
    if payload.description is not None: recipe.description = payload.description
    if payload.notes is not None: recipe.notes = payload.notes
    if payload.author is not None: recipe.author = payload.author
    if payload.type is not None: recipe.type = payload.type
    ...
    for f in payload.recipeFermentables or []:
        if f.id and f.id in existing_f_by_id:
            ...
            obj.potential_extract = f.potentialExtract if f.potentialExtract is not None else obj.potential_extract
