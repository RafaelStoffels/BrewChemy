...
from sqlalchemy import Integer, String, Text, Numeric, DateTime, Boolean, ForeignKey, Date, func
...
class Hop(Base):
    ...
    name: Mapped[str] = mapped_column(String(40), nullable=False)
    supplier: Mapped[str | None] = mapped_column(String(40), nullable=True)
    alpha_acid_content: Mapped[float | None] = mapped_column(Numeric(5, 2), nullable=True)
    beta_acid_content: Mapped[float | None] = mapped_column(Numeric(5, 2), nullable=True)
...
class Recipe(Base):
    ...
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    creation_date: Mapped[object] = mapped_column(Date, server_default=func.current_date())
    ...
    recipe_equipment: Mapped["RecipeEquipment | None"] = relationship(
        back_populates="recipe", cascade="all, delete-orphan", uselist=False, lazy="selectin"
    )
